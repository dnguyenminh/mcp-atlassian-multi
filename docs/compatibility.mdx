...
<Steps>
  ...
  <Step title="Verify tool discovery">
    If Copilot reports "Retrieved 0 tools", ensure you are using `mcp-atlassian-multi >= 0.16.0` (which includes FastMCP updates for better protocol compliance).
  </Step>
...
```bash
uvx mcp-atlassian-multi@latest
```

### ChatGPT

ChatGPT's MCP integration has reported vague "violates guidelines" errors. This may be related to schema validation but no specific diagnostics are available. If you encounter this, please [open an issue](https://github.com/sooperset/mcp-atlassian-multi/issues) with the exact error message.
