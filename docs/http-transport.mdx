...
```bash
# Using uvx
uvx mcp-atlassian-multi --transport streamable-http --stateless --port 9000 -vv
...
STATELESS=true uvx mcp-atlassian-multi --transport streamable-http --port 9000
```
...
<Tabs>
  <Tab title="SSE Transport">
    ```bash
    # Using uvx
    uvx mcp-atlassian-multi --transport sse --port 9000 -vv
  ...
  <Tab title="Streamable-HTTP">
    ```bash
    # Using uvx
    uvx mcp-atlassian-multi --transport streamable-http --port 9000 -vv
...
<Steps>
  <Step title="Run OAuth Setup (if using OAuth)">
    ```bash
    # Using uvx
    uvx mcp-atlassian-multi --oauth-setup -v

    # Or using Docker
    docker run --rm -i \
      -p 8080:8080 \
      -v "${HOME}/.mcp-atlassian-multi:/home/app/.mcp-atlassian-multi" \
      ghcr.io/sooperset/mcp-atlassian-multi:latest --oauth-setup -v
    ```
  </Step>
  ...
  <Step title="Configure Environment">
    ...
    PUBLIC_BASE_URL=https://mcp.example.com/mcp-atlassian-multi
    ```
  </Step>
</Steps>
...
1. Enable minimal OAuth mode:
   ```bash
   # Using uvx
   ATLASSIAN_OAUTH_ENABLE=true uvx mcp-atlassian-multi --transport streamable-http --port 9000
