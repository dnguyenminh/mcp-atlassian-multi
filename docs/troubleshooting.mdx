...
<AccordionGroup>
  ...
  <Accordion title="SSL Certificate Issues">
    mcp-atlassian-multi uses the OS native trust store by default (via
...
```bash
# With uvx
npx @modelcontextprotocol/inspector uvx mcp-atlassian-multi

# With local development version
npx @modelcontextprotocol/inspector uv --directory /path/to/mcp-atlassian-multi run mcp-atlassian-multi
```
...
<AccordionGroup>
  ...
  <Accordion title="OAuth Token Expired">
    ...
    **Fix:**
    1. Re-run the OAuth setup: `mcp-atlassian-multi --oauth-setup`
...
- Check [GitHub Issues](https://github.com/sooperset/mcp-atlassian-multi/issues) for known problems
- Review [SECURITY.md](https://github.com/sooperset/mcp-atlassian-multi/blob/main/SECURITY.md) for security-related concerns
