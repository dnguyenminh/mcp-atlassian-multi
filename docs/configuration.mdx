...
```json
{
  "mcpServers": {
    "mcp-atlassian-multi": {
      ...
      "args": [
        "run", "--rm", "-i",
        "--env-file", "/path/to/your/mcp-atlassian-multi.env",
...
See [.env.example](https://github.com/sooperset/mcp-atlassian-multi/blob/main/.env.example) for all available options.
...
```bash
uvx mcp-atlassian-multi --enabled-tools "confluence_search,jira_get_issue"
```
...
Command-line alternative:
```bash
uvx mcp-atlassian-multi --toolsets "default,jira_agile"
```
