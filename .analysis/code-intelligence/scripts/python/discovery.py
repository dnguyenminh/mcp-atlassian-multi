...
def discover_modules(root_dir: str, detection: dict) -> list[dict]:
    ...
    discoverers = {
        "gradle-kotlin": _discover_gradle,
        "gradle-java": _discover_gradle,
        "maven-java": _discover_maven,
        "npm-typescript": _discover_npm,
        "npm-javascript": _discover_npm,
        "cargo-rust": _discover_cargo,
        "go-module": _discover_go,
...
def _create_root_module(root_dir: str, project_type: str, build_file: str) -> dict:
    ...
    return {
        ...
        "buildFile": build_file if build_file != "none" else None,
        "language": None,
    }
...
def _detect_src_dirs(root_dir: str, mod_path: str, project_type: str) -> list[str]:
    ...
    for candidate in ["src", "lib", "."]:
        full = (
            os.path.join(root_dir, mod_path, candidate)
            if mod_path != "."
            else os.path.join(root_dir, candidate)
        )
...
def _gradle_maven_src(root_dir: str, mod_path: str) -> list[str]:
    ...
    candidates = [
        f"{mod_path}/src/main/kotlin",
        f"{mod_path}/src/main/java",
        f"{mod_path}/src/main/resources",
        f"{mod_path}/src/test/kotlin",
...
def _discover_maven(root_dir: str) -> list[dict]:
    ...
    if not os.path.isfile(pom):
        return modules
    content = open(pom, encoding="utf-8", errors="ignore").read()
            ...
            if os.path.isdir(parent_dir):
                for entry in os.scandir(parent_dir):
                    if entry.is_dir() and os.path.isfile(
                        os.path.join(entry.path, "package.json")
                    ):
                        ...
                        modules.append(
                            {
                                ...
                                "buildFile": f"{mod_path}/package.json",
                                "language": None,
                            }
                        )
            ...
            modules.append(
                {
                    ...
                    "buildFile": f"{pattern}/package.json",
                    "language": None,
                }
            )
    return modules
...
def _discover_cargo(root_dir: str) -> list[dict]:
    ...
    if not os.path.isfile(cargo):
        return []
    content = open(cargo, encoding="utf-8", errors="ignore").read()
...
def _discover_go(root_dir: str) -> list[dict]:
    ...
    if not os.path.isfile(go_mod):
        return []
    content = open(go_mod, encoding="utf-8", errors="ignore").read()
...
def _discover_python(root_dir: str) -> list[dict]:
    ...
    for indicator in ("packages", "libs"):
        ...
        for entry in os.scandir(ind_dir):
            if entry.is_dir():
                has_build = os.path.isfile(
                    os.path.join(entry.path, "pyproject.toml")
                ) or os.path.isfile(os.path.join(entry.path, "setup.py"))
                if has_build:
                    sub = f"{indicator}/{entry.name}"
                    src = (
                        f"{sub}/src"
                        if os.path.isdir(os.path.join(root_dir, sub, "src"))
                        else sub
                    )
                    modules.append(
                        {
