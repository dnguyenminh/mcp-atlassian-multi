...
def _parse_java(fp, content, mod):
    ...
    for line in content.split("\n"):
        ...
        if m and m.group(2) not in ("class", "interface", "enum"):
            functions.append(
                _fn(m.group(3), m.group(1) or "package-private", [], m.group(2))
            )
...
def _parse_python(fp, content, mod):
    ...
    for line in content.split("\n"):
        ...
        if m:
            classes.append(_cls(m.group(1)))
            continue
        m = re.match(r"^def\s+(\w+)\s*\(([^)]*)\)(?:\s*->\s*(\S+))?:", t)
        if m:
            vis = "private" if m.group(1).startswith("_") else "public"
            functions.append(
                _fn(m.group(1), vis, _params_python(m.group(2)), m.group(3) or "None")
            )
    return _success_result(
        fp, "python", mod, derive_package(fp), classes, functions, imports
    )
...
def _parse_go(fp, content, mod):
    ...
    classes = [
        _cls(m.group(1), "exported" if m.group(1)[0].isupper() else "unexported")
        for m in re.finditer(r"type\s+(\w+)\s+struct", content)
    ]
    functions = [
        _fn(m.group(1), "exported" if m.group(1)[0].isupper() else "unexported")
...
def _parse_rust(fp, content, mod):
    ...
    imports = [
        m.group(1) for m in re.finditer(r"^use\s+([\w:*]+);", content, re.MULTILINE)
    ]
    classes = [
        _cls(m.group(2), "pub" if m.group(1) else "private")
        for m in re.finditer(r"(pub\s+)?(?:struct|enum)\s+(\w+)", content)
    ]
    functions = [
        _fn(m.group(2), "pub" if m.group(1) else "private")
        for m in re.finditer(r"(pub\s+)?fn\s+(\w+)", content)
    ]
    return _success_result(
        fp, "rust", mod, derive_package(fp), classes, functions, imports
    )
...
def _parse_csharp(fp, content, mod):
    """Parse C# source file."""
    imports = [
        m.group(1) for m in re.finditer(r"^using\s+([\w.]+);", content, re.MULTILINE)
    ]
    ns_match = re.search(r"namespace\s+([\w.]+)", content)
    pkg = ns_match.group(1) if ns_match else ""
    classes = [
        _cls(m.group(1)) for m in re.finditer(r"(?:class|interface)\s+(\w+)", content)
    ]
    functions = [
        _fn(m.group(2), m.group(1) or "private", [], m.group(2))
        for m in re.finditer(r"(public|private|protected)?\s*\w+\s+(\w+)\s*\(", content)
        if m.group(2)
        not in ("class", "interface", "enum", "namespace", "if", "for", "while")
    ]
...
def _parse_typescript(fp, content, mod):
    ...
    classes = [
        _cls(m.group(2), "exported" if m.group(1) else "private")
        for m in re.finditer(r"(export\s+)?class\s+(\w+)", content)
    ]
    functions = [
        _fn(m.group(2), "exported" if m.group(1) else "private")
        for m in re.finditer(r"(export\s+)?(?:async\s+)?function\s+(\w+)", content)
    ]
    return _success_result(
        fp, "typescript", mod, derive_package(fp), classes, functions, imports
    )
...
def _parse_fallback(fp, content, mod):
    ...
    imports = [
        m.group(1).strip("'\";")
        for m in re.finditer(r'(?:import\s+|require\s*\(\s*[\'"])(\S+)', content)
    ]
...
def _params_colon(raw: str) -> list[dict]:
    ...
    for p in raw.split(","):
        ...
        parts = p.split(":", 1)
        result.append(
            _param(parts[0].strip(), parts[1].strip() if len(parts) > 1 else "Any")
        )
    return result
