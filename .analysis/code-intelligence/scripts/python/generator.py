...
def generate_project_structure(
    modules_data: list[dict], project_info: dict, output_dir: str
):
...
def generate_module_analysis(module_data: dict, output_dir: str):
    ...
    if pub_fns:
        for fn in pub_fns[:20]:
            params = ", ".join(
                f"{p['name']}: {p['type']}" for p in fn.get("parameters", [])
            )
    ...
    atomic_write(
        os.path.join(modules_dir, f"{module_data['name']}.md"), "\n".join(lines)
    )
